...
{
    Medusa::UmengAnalyzer* Analyzer;
}
-(void)OnReady:(NSNotification*)notification;
-(id)initWithAnalyzer:(Medusa::UmengAnalyzer*)analyzer;
@end
...
-(void)OnReady:(NSNotification*)notification
{
    Medusa::AnalyticsOnlineConfigDelegate callback= Analyzer->GetOnlineConfigDelegate();
    callback();
}
...
bool UmengAnalyzer::Uninitialize()
{
    if (mNotificationHost!=NULL) {
        [[NSNotificationCenter defaultCenter] removeObserver:(UmengOnlineCallbackHandler*)mNotificationHost name:UMOnlineConfigDidFinishedNotification object:nil];
        [(UmengOnlineCallbackHandler*)mNotificationHost dealloc];
        mNotificationHost=NULL;
    }
...
{
    StringParser::ResultStringType str=StringParser::ToString(level);
