...
int CScriptBuilder::BuildModule(bool isRebuild/*=true*/)
{
	return Build(isRebuild);
}
...
bool CScriptBuilder::IsIncluded( const char *filename ) const
{
	std::string scriptFile = filename;
	return includedScripts.find(scriptFile) != includedScripts.end();
}
...
int CScriptBuilder::Build(bool isRebuild/*=true*/)
{
	int r = module->Build(isRebuild);
	if( r < 0 )
		return r;
