...
{
	Init(strDateTime, "%d-%d-%d %d:%d:%d",isUTC);
}
...
DateTime::DateTime( const DateTime& dateTime )
	:mDate(dateTime.mDate),mTotalSeconds(dateTime.mTotalSeconds)
{
...
{
	return TryParse(strDateTime, "%d-%d-%d %d:%d:%d",isUTC);
}
...
Medusa::DateTime DateTime::Now()
{
	return DateTime(time(NULL),false);
}

Medusa::DateTime DateTime::UTCNow()
{
	return DateTime(time(NULL),true);
}
...
{
#ifdef MEDUSA_WINDOWS
	errno_t err= gmtime_s(&mDate,&mTotalSeconds);
#else
...
{
#ifdef MEDUSA_WINDOWS
	errno_t err= localtime_s(&mDate,&mTotalSeconds);
#else
...
void DateTime::PrintTimeZone()
{
	...
	Log::LogInfoFormat("IsDayLight:%s\tTimeZone:%ldd\tTimeZoneName:%s",StringParser::ToString(IsDayLight()).c_str(),GetTimeZoneSeconds(),timeZoneName.c_str());
}
