...
Medusa::Vector3F Matrix43::TransformVector( const Vector3F& point ) const
{
...
HeapString Matrix43::ToString() const
{
	...
	*/
	...
	HeapString result;
	result.Format("[%f,%f,%f]\n[%f,%f,%f]\n[%f,%f,%f]\n[%f,%f,%f]",M11,M12,M13,M21,M22,M23,M31,M32,M33,M41,M42,M43);
	return result;
...
Matrix43& Matrix43::RotateX( float angle)
{
...
Matrix43& Matrix43::RotateY( float angle)
{
...
Matrix43& Matrix43::RotateZ( float angle)
{
...
Matrix43 Matrix43::CreateRotateX( float angle )
{
	float cosX=Math::Cos(angle);
	float sinX=Math::Sin(angle);
...
Matrix43 Matrix43::CreateRotateY( float angle )
{
	float cosY=Math::Cos(angle);
	float sinY=Math::Sin(angle);
...
Matrix43 Matrix43::CreateRotateZ( float angle )
{
	float cosZ=Math::Cos(angle);
	float sinZ=Math::Sin(angle);
...
Matrix43 Matrix43::CreateRotateAxis( const Point3F& axis,float angle )
{
...
Medusa::Matrix43 Matrix43::CreateNormal()
{
