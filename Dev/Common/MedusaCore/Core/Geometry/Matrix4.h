...
(T*R*S)-t等价于S-t*R-t*T-t ，所以因为我们是按行储存S*R*T的，当OpenGL把它按照列来读取的时候，实际上就相当于读取（S*R*T）-t,等价于是T-t*R-t*S-t，
又因为我们中间运算过程中的S,R,T都是按照行储存的，所以每一个SRT都相当于做了一次-t，所以最终结果就会正确。
*/
...
public:
	...
	Rect2F TransformRect(const Rect2F& rect)const;
	CubeF TransformCube(const CubeF& cube)const;
...
#pragma region Transform
	...
	Matrix4& ResetWorld(const Scale3F& scale,const Rotation3F& rotation,const Point3F& translation,const Point3F& pivot=Point3F::Zero);

	Matrix4 CreateInverse();
	Matrix43 CreateInverse43();

	//************************************
	// 名称:    	InverseMatrixByGauss
	// 返回:   	Matrix44	
	// 功能:		用高斯-约旦法求解
	// 描述:		
	// 例子:		
	...
	static Matrix4 CreateRotateAxis(const Point3F& axis,float angle);
	...
	static Matrix4 CreateWorld(const Scale3F& scale,const Rotation3F& rotation,const Point3F& translation,const Point3F& pivot=Point3F::Zero);
