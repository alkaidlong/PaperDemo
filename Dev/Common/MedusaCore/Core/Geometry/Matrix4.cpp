...
Medusa::Vector3F Matrix4::TransformVector( const Vector3F& point ) const
{
	//prepend
...
Medusa::Point2F Matrix4::TransformPoint( const Point2F& point ) const
{
...
Point3F Matrix4::TransformPoint( const Point3F& point)const
{
...
Point4F Matrix4::TransformPoint( const Point4F& point ) const
{
...
Medusa::Rect2F Matrix4::TransformRect(const Rect2F& rect) const
{
	Point2F p1=TransformPoint(rect.LeftBottom());
	Point2F p2=TransformPoint(rect.LeftTop());
	Point2F p3=TransformPoint(rect.RightBottom());
	Point2F p4=TransformPoint(rect.RightTop());

	float minX=Math::Min4(p1.X,p2.X,p3.X,p4.X);
	float maxX=Math::Max4(p1.X,p2.X,p3.X,p4.X);
	float minY=Math::Min4(p1.Y,p2.Y,p3.Y,p4.Y);
	float maxY=Math::Max4(p1.Y,p2.Y,p3.Y,p4.Y);

	return Rect2F(minX,minY,maxX-minX,maxY-minY);
}

Medusa::CubeF Matrix4::TransformCube(const CubeF& cube) const
{
	if (cube.IsZeroDepth())
	{
		Point3F p1=TransformPoint(cube.LeftBottomNear());
		Point3F p2=TransformPoint(cube.LeftTopNear());
		Point3F p3=TransformPoint(cube.RightBottomNear());
		Point3F p4=TransformPoint(cube.RightTopNear());
	...
	else
	{
		...
		Point3F p5=TransformPoint(cube.LeftBottomFar());
		Point3F p6=TransformPoint(cube.LeftTopFar());
		Point3F p7=TransformPoint(cube.RightBottomFar());
		Point3F p8=TransformPoint(cube.RightTopFar());

		float minX=Math::Min8(p1.X,p2.X,p3.X,p4.X,p5.X,p6.X,p7.X,p8.X);
		float maxX=Math::Max8(p1.X,p2.X,p3.X,p4.X,p5.X,p6.X,p7.X,p8.X);

		float minY=Math::Min8(p1.Y,p2.Y,p3.Y,p4.Y,p5.Y,p6.Y,p7.Y,p8.Y);
		float maxY=Math::Max8(p1.Y,p2.Y,p3.Y,p4.Y,p5.Y,p6.Y,p7.Y,p8.Y);

		float minZ=Math::Min8(p1.Z,p2.Z,p3.Z,p4.Z,p5.Z,p6.Z,p7.Z,p8.Z);
		float maxZ=Math::Max8(p1.Z,p2.Z,p3.Z,p4.Z,p5.Z,p6.Z,p7.Z,p8.Z);
...
HeapString Matrix4::ToString() const
{
	...
	*/
	...
	HeapString result;
	result.Format("[%f,%f,%f,%f]\n[%f,%f,%f,%f]\n[%f,%f,%f,%f]\n[%f,%f,%f,%f]",M11,M12,M13,M14,M21,M22,M23,M24,M31,M32,M33,M34,M41,M42,M43,M44);
	return result;
...
{
	return Memory::Equals(M,matrix.M);
}
...
Matrix4& Matrix4::ResetWorld( const Scale3F& scale,const Rotation3F& rotation,const Point3F& translation,const Point3F& pivot/*=Point3F::Zero*/ )
{
	*this=CreateWorld(scale,rotation,translation,pivot);
	return *this;
}
...
Medusa::Matrix43 Matrix4::CreateInverse43()
{
	//			Should be		[A 0]
	//							[B 1]
...
Matrix4 Matrix4::CreateInverseByGauss()
{
	...
	1.从第 k 行、第 k 列开始的右下角子阵中选取绝对值最大的元素，并记住元素所在的行号和列号，在通过行交换和列交换将它交换到主元素位置上。这一步称为全选主元。
	2.m(k, k) = 1 / m(k, k)
...
Matrix4 Matrix4::CreateRotateAxis( const Point3F& axis,float angle )
{
...
Medusa::Matrix4 Matrix4::CreateWorld( const Scale3F& scale,const Rotation3F& rotation,const Point3F& translation,const Point3F& pivot/*=Point3F::Zero*/ )
{
	//旋转顺序为 x,y,z
	if (rotation!=Rotation3F::Zero)
	{
		...
		return Matrix4(m11,m12,m13,0.f,
			m21,m22,m23,0.f,
			m31,m32,m33,0.f,
			m41,m42,m43,1.f);
	}
	else
	{
		return Matrix4(scale.X,0.f,0.f,0.f,
			0.f,scale.Y,0.f,0.f,
			0.f,0.f,scale.Z,0.f,
			translation.X-pivot.X,translation.Y-pivot.Y,translation.Z-pivot.Z,1.f);
	}
...
Medusa::Matrix4 Matrix4::CreateNormal()
{
...
{
	//左右手坐标系的区别在于Z轴是否要取反,所以对于opengl，就是第3列取反，对于directX，就是第3行取反
	float widthRcp=1.f/(right-left);
	float heightRcp=1.f/(top-bottom);
	float depthRcp=1.f/(farPlane-nearPlane);
...
Medusa::Matrix4 Matrix4::CreateBillboard( const Vector3F& objectPosition,const Vector3F& cameraPosition, const Vector3F& cameraUp, const Vector3F& cameraForward/*=Vector3F::ForWard*/,
...
Medusa::Matrix4 Matrix4::CreateConstrainedBillboard( const Vector3F& objectPosition,const Vector3F& cameraPosition, const Vector3F& rotateAxis, const Vector3F& cameraForward/*=Vector3F::ForWard*/,const Vector3F& objectForward/*=Vector3F::ForWard*/,bool isRightHand/*=MEDUSA_IS_RIGHT_HAND*/,GraphicsSDK sdk/*=MEDUSA_DEFAULT_GRAPHICS_SDK*/ )
{
	//TODO: test
...
{
	//受限欧拉角。这里将y限制在正负90度，x、z限制在正负180度
