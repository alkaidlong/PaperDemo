...
public:
	...
	static void InsertSort(ICollection<T>& items)
	{
		...
		if (items.IsLinear())
		{
			InsertSortHelper<T,TCompare>(items.GetItems(),0,items.GetCount()-1);
		}
		else
		{
			MEDUSA_ASSERT_FAILED("");	//not implement
	...
	template<typename T,typename TCompare>
	static intp BinarySearchFirst(typename InnerTypes<T>::TPointerType items,typename InnerTypes<T>::TParameterType key,size_t min,size_t max)
	{
	...
	template<typename T,typename TCompare>
	static intp BinarySearchLast(typename InnerTypes<T>::TPointerType items,typename InnerTypes<T>::TParameterType key,size_t min,size_t max)
	{
		//http://en.wikipedia.org/wiki/Binary_search_algorithm
		//Deferred detection of equality
		// continually narrow search until just one element remains
	...
	template<typename T,typename TCompare>
	static intp BinarySearchRandom(typename InnerTypes<T>::TPointerType items,typename InnerTypes<T>::TParameterType key,size_t min,size_t max)
	{
		//http://en.wikipedia.org/wiki/Binary_search_algorithm
		//Match the first one
		while(min<max)
	...
	template<typename T,typename TCompare>
	static intp BinarySearchLastGreaterThan(typename InnerTypes<T>::TPointerType items,typename InnerTypes<T>::TParameterType key,size_t min,size_t max)
	{
	...
	static void QuickSortHelper(typename InnerTypes<T>::TPointerType items,size_t left,size_t right)
	{
		...
		size_t i=left;
		size_t j=right;
		T mid= items[(left + right) / 2]; //以靠近中间值为主元
		do  
		{  
		...
		{
			QuickSortHelper<T,TCompare>(items,left,i-1);     //对标准值左半部递归调用本函数  
		}
		if ((size_t)i+1<right)
		{
			QuickSortHelper<T,TCompare>(items,i+1,right);    //对标准值右半部递归调用本函数  
		}
