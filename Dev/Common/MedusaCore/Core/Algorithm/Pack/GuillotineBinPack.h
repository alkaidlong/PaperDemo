...
public:
	...
	GuillotineBinPack(const Size2U& size);
	...
	Rect2U Insert(const Size2U& size, bool merge, FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod);
	...
	/// @param rectChoice The free rectangle choice heuristic rule to use.
	/// @param splitMethod The free rectangle split heuristic rule to use.
	void Insert(List<Size2U> &sizes, bool merge, FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod);

// Implements GUILLOTINE-MAXFITTING, an experimental heuristic that's really cool but didn't quite work in practice.
//	void InsertMaxFitting(List<Size2U> &rects, List<Rect2U> &dst, bool merge, 
//		FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod);
	...
	List<Rect2U>& MutableFreeRects() { return mFreeRects; }

	/// Returns the list of packed rectangles. You may alter this vector at will, for example, you can move a Rect2U from
	/// this list to the Free Rectangles list to free up space on-the-fly, but notice that this causes fragmentation.
	const List<Rect2U>& GetUsedRects() { return mUsedRects; }
...
private:
	...
	///		rect was placed.
	/// @return A Rect2U structure that represents the placement of the new rect into the best free rectangle.
	Rect2U FindPositionForNewNode(const Size2U& size, FreeRectChoiceHeuristic rectChoice, int& outNodeIndex);

	static int ScoreByHeuristic(const Size2U& size, const Rect2U &freeRect, FreeRectChoiceHeuristic rectChoice);
	...
	static int ScoreBestAreaFit(const Size2U& size, const Rect2U &freeRect);
	static int ScoreBestShortSideFit(const Size2U& size, const Rect2U &freeRect);
	static int ScoreBestLongSideFit(const Size2U& size, const Rect2U &freeRect);

	static int ScoreWorstAreaFit(const Size2U& size, const Rect2U &freeRect);
	static int ScoreWorstShortSideFit(const Size2U& size, const Rect2U &freeRect);
	static int ScoreWorstLongSideFit(const Size2U& size, const Rect2U &freeRect);
	...
	void SplitFreeRectByHeuristic(const Rect2U &freeRect, const Rect2U &placedRect, GuillotineSplitHeuristic method);

	/// Splits the given L-shaped free rectangle into two new free rectangles along the given fixed split axis.
	void SplitFreeRectAlongAxis(const Rect2U &freeRect, const Rect2U &placedRect, bool splitHorizontal);
