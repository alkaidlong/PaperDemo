...
void GuillotineBinPack::Insert(List<Size2U> &sizes, bool isMerge, FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod)
{
	...
	while(!sizes.IsEmpty())
	{
		...
		size_t freeRectCount=mFreeRects.GetCount();
		size_t rectCount=sizes.GetCount();
		FOR_EACH_SIZE(i,freeRectCount)
		{
			FOR_EACH_SIZE(j,rectCount)
			{
				...
				if (sizes[j] == mFreeRects[i].Size)
				{
					...
					i = mFreeRects.GetCount(); // Force a jump out of the outer loop as well - we got an instant fit.
					break;
				}
...
int fitBottom = 0;
int fitRight = 0;
for(size_t i = 0; i < rects.GetCount(); ++i)
if (i != usedRectIndex)
{
...
while(rects.GetCount() > 0 && freeRectangles.GetCount() > 0)
{
int bestScore1 = -1;
int bestScore2 = -1;

///\todo Different sort predicates.
clb::sort::QuickSort(&freeRectangles[0], freeRectangles.GetCount(), CompareRectShortSide);

Rect2U &freeRect = freeRectangles[0];

for(size_t j = 0; j < rects.GetCount(); ++j)
{
int score1;
int score2;
...
float GuillotineBinPack::Occupancy() const
{
	...
	size_t usedCount=mUsedRects.GetCount();
...
Rect2U GuillotineBinPack::FindPositionForNewNode(const Size2U& size, FreeRectChoiceHeuristic rectChoice, int& outNodeIndex)
{
	...
	size_t freeCount=mFreeRects.GetCount();
...
void GuillotineBinPack::MergeFreeList()
{
	...
	for(size_t i = 0; i < mFreeRects.GetCount(); ++i)
	{
		for(size_t j = i+1; j < mFreeRects.GetCount(); ++j)
		{
