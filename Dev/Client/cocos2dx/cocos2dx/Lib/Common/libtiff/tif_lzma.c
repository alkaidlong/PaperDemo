...
static int LZMAEncode(TIFF* tif, TIFF_UINT8_T* bp, tmsize_t cc, TIFF_UINT16_T s);
static int LZMADecode(TIFF* tif, TIFF_UINT8_T* op, tmsize_t occ, TIFF_UINT16_T s);
 ...
 */
static int
LZMAPreDecode(TIFF* tif, TIFF_UINT16_T s)
{
...
static int
LZMADecode(TIFF* tif, TIFF_UINT8_T* op, tmsize_t occ, TIFF_UINT16_T s)
{
	...
	if (sp->stream.avail_out != 0) {
        ...
        tif->tif_rawcp = (TIFF_UINT8_T *)sp->stream.next_in; /* cast away const */
 ...
 */
static int
LZMAPreEncode(TIFF* tif, TIFF_UINT16_T s)
{
 ...
 */
static int
LZMAEncode(TIFF* tif, TIFF_UINT8_T* bp, tmsize_t cc, TIFF_UINT16_T s)
{
...
static int
LZMAVSetField(TIFF* tif, TIFF_UINT32_T tag, va_list ap)
{
...
static int
LZMAVGetField(TIFF* tif, TIFF_UINT32_T tag, va_list ap)
{
	LZMAState* sp = LState(tif);
...
TIFFInitLZMA(TIFF* tif, int scheme)
{
	 ...
	 */
	tif->tif_data = (TIFF_UINT8_T*) _TIFFmalloc(sizeof(LZMAState));
