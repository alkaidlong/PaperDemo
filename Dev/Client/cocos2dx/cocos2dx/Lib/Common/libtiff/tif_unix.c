...
static TIFF_UINT64_T
_tiffSeekProc(thandle_t fd, TIFF_UINT64_T off, int whence)
{
	off_t off_io = (off_t) off;
	if ((TIFF_UINT64_T) off_io != off)
	{
		errno=EINVAL;
		return (TIFF_UINT64_T) -1; /* this is really gross */
	}
	return((TIFF_UINT64_T)lseek((int)fd,off_io,whence));
}
...
_tiffSizeProc(thandle_t fd)
{
	...
	if (fstat((int)fd,&sb)<0)
		...
		return((TIFF_UINT64_T)sb.st_size);
}
...
{
	TIFF_UINT64_T size64 = _tiffSizeProc(fd);
	tmsize_t sizem = (tmsize_t)size64;
	if ((TIFF_UINT64_T)sizem==size64) {
		*pbase = (void*)
