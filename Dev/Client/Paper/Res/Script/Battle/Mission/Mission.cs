    ...
    void UpdateRoles(float dt)
    {
        for(uint i = 0;i<mLeftRolses.length;i++)
        {
            ...
            else
            {
                ...
                if(mTime>10.0f &&!mTemp && i == 1)//测试角色突然跳转摄像机工作是否正常
                {
                    mTemp = true;
            ...
            for( uint i = 0 ; i<mLeftRolses.length; i++ )
            {
                ...
                if( role!=null && role.IsLive())
                {
                    ...
                    if( cur > max )
                    {
                        ...
                        if(i!=0)
                        {
                            Role@ role0 = mLeftRolses[0];

                            mLeftRolses.removeAt(0);
                            mLeftRolses.insertAt(0,role);
                            mLeftRolses.removeAt(i);
                            mLeftRolses.insertAt(i,role0);
                        }
                    }
                }
            }
